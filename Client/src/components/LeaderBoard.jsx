...
const LeaderBoard = () => {
  ...
  return (
    <>
        <div className={`absolute top-0 left-0 h-fit z-10 ${darkMode ? 'bg-[#121213] text-white' : 'bg-white text-black'}`}>
            <div className='w-[500px] sm:w-screen mx-auto'>
                <div className='w-[500px] sm:w-screen fixed top-0 left-0 right-0 mx-auto bg-[#121213] z-10'>
                ...
                <div className='mt-[110px] border-2 border-[#787c7e] rounded-md pb-4 mb-2'>
                    ...
                    <div>
                        <div className='grid grid-cols-7 gap-2 px-2'>
                            ...
                            {displayed.length > 0 && displayed.map((val, ind) => (
                                <>
                                    {[ind + 1, val.username, val.highest, val.streak, val.played].map((leaderboardVal,arrInd) => 
                                    (
                                        <div className={`text-center select-none col-span-1 border-transparent ${leaderboardVal === val.username && 'col-span-3'} rounded-md border-2 ${(val.id == userID.id ) && (darkMode ? '!border-white' : '!border-transparent')} `}>
                                            <button className={`block w-full ${ 
                                                (
                                                arrInd === 2 && highestButton ||
                                                arrInd === 3 && streakButton ||
                                                arrInd === 4 && playedButton
                                                ) ?
                                                'opacity-100'
                                                :
                                                'opacity-70'
                                                } !text-black cursor-default select-none col-span-1 ${leaderboardVal === val.username && 'col-span-3'} text-center font-bold rounded-md border-2 border-transparent ${(val.id == userID.id ) && (darkMode ? '!border-white' : '!border-black')} ${
